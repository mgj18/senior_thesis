...
with open(paf_file) as file:
    for alignment in file:
        ...
        try:
            index = chromosomes.index(true_chr_arm_hap)
        except ValueError:
            continue

        if mapped_chr_arm_hap not in chromosomes:
            continue

        if map_Q >= 60:
            df60.loc[index,mapped_chr_arm_hap] += 1
        elif map_Q >= 15:
            df15.loc[index,mapped_chr_arm_hap] += 1
        elif map_Q >= 1:
            df1.loc[index,mapped_chr_arm_hap] += 1
        else:
            df0.loc[index,mapped_chr_arm_hap] += 1
